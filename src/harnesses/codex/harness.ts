...
function getDirectory(global?: boolean) {
  if (global) {
    return join(process.env.CODEX_HOME ?? join(UserHome, ".codex"), "agents");
  }
...
const listFiles = async (location: string) => {
  ...
  const paths = await glob("*.toml", { cwd: location, absolute: true });
  ...
  apply: async (options) => {
    ...
    await Promise.all(
      AgentList.map(async (agent) => {
        const path = join(directory, `${agent.name}.toml`);
